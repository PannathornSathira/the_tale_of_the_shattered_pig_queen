...
class BlueDragonBoss(BaseBoss):
    def __init__(self, x, y, health=300):
        ...
        self.stomp_effect_duration = 0.3
        self.stomp_effect_time = 0
        self.stomp_effect_isVisible = False
        self.stomp_effect_rect1 = pygame.Rect(0, GROUND_LEVEL_Y, WIDTH, 20)  # Define the size and position of the effect
        self.stomp_effect_rect2 = pygame.Rect(self.x + self.width / 2 - self.stomp_stun_distance, GROUND_LEVEL_Y, self.stomp_stun_distance * 2, 20)  # Define the size and position of the effect
    ...
    def glacial_shards(self, dt, player):
        ...
        if self.ice_summon_gap_time >= self.ice_summon_gap_cooldown:
            ...
            ice_shard = IceShard(spawn_x, spawn_y)
...
class IceShard:
    ...
    def explode(self, boss):
        ...
        for i in range(num_bullets):
            ...
            bullet = BossBullet(self.x, self.y, "general", self.damage // 2, (bullet_dx, bullet_dy))  # Reduce bullet damage
